...
pub async fn spawn_udp_forwarder(
    listen_addr: &str,
    state: Arc<RwLock<super::AppState>>,
) -> Result<()> {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let backend_addr_opt = {
                let state_guard = state.read().await;
                if let Some(b) = state_guard.udp_client_map.get(&src.ip()).cloned() {
                    Some(b)
                } else {
                    state_guard.config.default.as_ref().map(|d| d.address.clone())
                }
            };
